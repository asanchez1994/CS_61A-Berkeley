...
class Ant(Insect):
    ...
    def add_to(self, place):
        ...
        if place.ant is None:
            ...
            # if ant in place is a bodyguard ant
            place.ant.contain_ant(self)
        elif isinstance(self, ContainerAnt) and self.contained_ant is None and not isinstance(place.ant, ContainerAnt):
            # if ant being placed is a body guard ant
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while (location.entrance is not None):
            if  self.min_range <= self.loops <= self.max_range: 
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        o_length = len(bee_list)

        # modify in place 
        while i < len(bee_list):
            if self.armor - amount <= 0:
                bee_list[i].reduce_armor(amount + self.damage)
            else:
                bee_list[i].reduce_armor(amount)
            if len(bee_list) ==  o_length:
                i += 1 
            else:
                o_length = len(bee_list)
        ...
        bee_list = self.place.bees[:]
        for elem in bee_list:
            elem.reduce_armor(amount + death_bonus)
        """
...
class NinjaAnt(Ant):
    ...
    def action(self, gamestate):
        ...
        while i < o_len:
            bee_list[i].reduce_armor(self.damage)
            if o_len == len(bee_list):
                i += 1
            else:
                o_len = len(bee_list)
        # END Problem 7
...
class ContainerAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        if self.contained_ant is None and not isinstance(ContainerAnt, other):
            return True
        return False
        # END Problem 9
...
class BodyguardAnt(ContainerAnt):
    ...
    def __init__(self, *args, **kwargs):
        Ant.__init__(self, *args, **kwargs)
        self.armor = self.body_guard_armor
        self.contained_ant = None
...
# END Problem 13
    ...
    def __init__(self, armor=1):
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if QueenAnt.true_queen_alive == False:
            self.impostor = False
            QueenAnt.true_queen_alive = True
        else:
            self.impostor = True
    ...
    def action(self, gamestate):
        ...
        # True queen action
        elif self.place.exit:
            # Do normal scuba throw 
            ScubaThrower.action(self, gamestate)

            # double the others
            behind_the_queen = self.place.exit
            while behind_the_queen:

                curr_ant = behind_the_queen.ant

                # Improve ants behind 
                if curr_ant and not curr_ant.already_doubled:
                    curr_ant.damage *= 2
                    curr_ant.already_doubled = True

                # Container ant special treatment
                if isinstance(curr_ant, ContainerAnt):
                    if curr_ant.contained_ant and not curr_ant.contained_ant.already_doubled:
                        curr_ant.contained_ant.damage *= 2
                        curr_ant.contained_ant.already_doubled = True


                behind_the_queen = behind_the_queen.exit
