...
def totals_tree(m):
    ...
    if is_planet(m):
        ...
        return tree(sum([label(x) for x in branches]), branches)
...
def replace_leaf(t, old, replacement):
    ...
    if is_leaf(t) and label(t) == old:
        ...
        branches_new = [replace_leaf(x, old, replacement) for x in branches(t)]
...
def make_withdraw(balance, password):
    ...
    "*** YOUR CODE HERE ***"
    incorrect_list = []
    def withdraw(amount, pass_arg):
        nonlocal balance
        nonlocal password
        nonlocal incorrect_list 
        
        if len(incorrect_list) == 3: 
            return 'Your account is locked. Attempts: ' + str(incorrect_list)
        elif password != pass_arg:
            incorrect_list.append(pass_arg)
...
def make_joint(withdraw, old_pass, new_pass):
    ...
    """
    "*** YOUR CODE HERE ***"
    mistake = withdraw(0, old_pass)
    if type(mistake) == str:
        return mistake  
    def joint_account(amount, attempt):
        if attempt == new_pass:
