...
def closer_city(lat, lon, city1, city2):
    ...
    dist1 = distance(test, city1) 
    dist2 = distance(test, city2) 
    if dist1 < dist2:
...
def sprout_leaves(t, values):
    ...
    return tree(label(t), [sprout_leaves(x, values) for x in branches(t)])
...
def add_chars(a, b):
    ...
    def helper(a, b, string=""):
        if a == b:
            ...
            if a[0] == b[0]:
                return helper(a[1:], b[1:], string)
            else:
                return helper(a, b[1:], string + b[0])
